// Code generated by go-swagger; DO NOT EDIT.

package ip_sets

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"
)

// New creates a new ip sets API client.
func New(transport runtime.ClientTransport, formats strfmt.Registry) ClientService {
	return &Client{transport: transport, formats: formats}
}

/*
Client for ip sets API
*/
type Client struct {
	transport runtime.ClientTransport
	formats   strfmt.Registry
}

// ClientService is the interface for Client methods
type ClientService interface {
	IPSetGetUsingGET(params *IPSetGetUsingGETParams) (*IPSetGetUsingGETOK, error)

	IPSetListUsingGET(params *IPSetListUsingGETParams) (*IPSetListUsingGETOK, error)

	SetTransport(transport runtime.ClientTransport)
}

/*
  IPSetGetUsingGET gets

  Get IPSet from NSX-T
*/
func (a *Client) IPSetGetUsingGET(params *IPSetGetUsingGETParams) (*IPSetGetUsingGETOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewIPSetGetUsingGETParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "ipSetGetUsingGET",
		Method:             "GET",
		PathPattern:        "/network/ipsets/{ipSetId}",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &IPSetGetUsingGETReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	})
	if err != nil {
		return nil, err
	}
	success, ok := result.(*IPSetGetUsingGETOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for ipSetGetUsingGET: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}

/*
  IPSetListUsingGET lists

  List IPSets from NSX-T
*/
func (a *Client) IPSetListUsingGET(params *IPSetListUsingGETParams) (*IPSetListUsingGETOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewIPSetListUsingGETParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "ipSetListUsingGET",
		Method:             "GET",
		PathPattern:        "/network/ipsets",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &IPSetListUsingGETReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	})
	if err != nil {
		return nil, err
	}
	success, ok := result.(*IPSetListUsingGETOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for ipSetListUsingGET: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}

// SetTransport changes the transport on the client
func (a *Client) SetTransport(transport runtime.ClientTransport) {
	a.transport = transport
}
